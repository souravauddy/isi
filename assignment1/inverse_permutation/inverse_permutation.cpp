...
bool valid_permutation(std::vector<int> &permutation) {
    const int N = int(permutation.size());
    std::vector<bool> count(N, 0);

    for (int i = 0; i < N; i++)
        count[permutation[i]] = 1;
    
    return std::accumulate(count.begin(), count.end(), 0) == N;
}
...
int main() {
    ...
    assert(valid_permutation(permutation));
